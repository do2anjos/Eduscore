...
# Carregar 01_corrigir_perspectiva (A4 Detection)
spec = importlib.util.spec_from_file_location("perspective_correction", script_dir / "01_corrigir_perspectiva.py")
perspective_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(perspective_module)
processar_imagem_a4 = perspective_module.processar_imagem_a4
carregar_modelos = perspective_module.carregar_modelos
...
def processar_imagem_enem_mobile(caminho_imagem):
    ...
    Args:
        caminho_imagem: Caminho da imagem capturada ou np.ndarray
    ...
    try:
        ...
        # 1. Correção de Perspectiva (YOLO -> SAM2)
        print("[ENEM-MOBILE] Iniciando pipeline de correção de perspectiva YOLO->SAM2...", file=sys.stderr)
        img_input = caminho_imagem
        
        try:
            # Carregar modelos (lazy load com cache)
            modelos = carregar_modelos()
            
            # Executar correção A4
            img_corrigida = processar_imagem_a4(caminho_imagem, modelos=modelos)
            
            if img_corrigida is not None:
                print("[ENEM-MOBILE] Correção de perspectiva A4 concluída com sucesso.", file=sys.stderr)
                img_input = img_corrigida
            else:
                print("[ENEM-MOBILE] FALHA na correção de perspectiva A4 (retornou None). Fallback para imagem original.", file=sys.stderr)
                # Salvar a imagem falha para telemetria/re-anotação
                try:
                    import time
                    import shutil
                    falha_dir = script_dir.parent.parent.parent / "logs" / "falhas_perspectiva"
                    os.makedirs(falha_dir, exist_ok=True)
                    timestamp = int(time.time())
                    if isinstance(caminho_imagem, (str, Path)) and os.path.exists(caminho_imagem):
                        shutil.copy(caminho_imagem, falha_dir / f"falha_a4_{timestamp}.jpg")
                    elif isinstance(caminho_imagem, np.ndarray):
                        cv2.imwrite(str(falha_dir / f"falha_a4_{timestamp}.jpg"), caminho_imagem)
                except Exception as e_log:
                    print(f"[ENEM-MOBILE] Erro ao salvar log de falha: {e_log}", file=sys.stderr)
        except Exception as e:
            print(f"[ENEM-MOBILE] ERRO CRÍTICO na correção de perspectiva A4: {e}", file=sys.stderr)
            traceback.print_exc()
            print("[ENEM-MOBILE] Fallback para imagem original.", file=sys.stderr)

        # 2. Detectar ROIs usando YOLO
        print("[ENEM-MOBILE] Detectando ROIs com YOLO...", file=sys.stderr)
        resultado_yolo = detect_rois(img_input)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(json.dumps({
            'sucesso': False, 
            'erro': 'Uso: python 05_processar_respostas.py <caminho_imagem>'
        }))
        sys.exit(1)
