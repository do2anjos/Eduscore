...
# Buscar modelo em huggingface-space/
SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = SCRIPT_DIR.parent.parent.parent  # Sobe 3 níveis a partir de Enem: Enem -> scripts -> backend -> raiz
MODEL_PATH = PROJECT_ROOT / "huggingface-space" / "best_yolo11s_optimized.onnx"
...
def load_model():
    ...
    if _NET is None:
        ...
        try:
            ...
            # OpenCV 5 removeu suporte a setPreferableTarget para o engine padrão
...
def postprocess_detections(outputs, original_shape, scale, pad):
    ...
    for row in output:
        ...
        if conf > CONFIDENCE_THRESHOLD:
            ...
            x_norm = float(x0 / width_orig)
            y_norm = float(y0 / height_orig)
            w_norm = float(w0 / width_orig)
            h_norm = float(h0 / height_orig)
...
def detect_enem_sheet(image_input):
    try:
        ...
        if len(answer_dets) > 0:
            ...
            min_xn = float(min([d['bbox_norm'][0] for d in answer_dets]))
            min_yn = float(min([d['bbox_norm'][1] for d in answer_dets]))
            max_xn = float(max([d['bbox_norm'][0] + d['bbox_norm'][2] for d in answer_dets]))
            max_yn = float(max([d['bbox_norm'][1] + d['bbox_norm'][3] for d in answer_dets]))
...
def detect_rois(caminho_imagem):
    """
    Wrapper para uso via CLI. Detecta ROIs e retorna crops em Base64.
    """
    import json
    
    # Carregar imagem
    image_bgr = load_image_robust(caminho_imagem)
    
    # Executar detecção
    resultado = detect_enem_sheet(image_bgr)
    
    if not resultado.get('sucesso') or not resultado.get('detectado'):
        return resultado
    
    rois = resultado.get('rois', {})
    recortes_base64 = {}
    
    # Gerar crops para cada ROI detectada
    if 'day_region' in rois and len(rois['day_region']) > 0:
        bbox = rois['day_region'][0]['bbox']
        x, y, w, h = bbox
        ...
        if crop.size > 0:
            ...
            recortes_base64['day_region'] = base64.b64encode(buffer).decode('utf-8')
    
    if 'answer_area_enem' in rois and len(rois['answer_area_enem']) > 0:
        bbox = rois['answer_area_enem'][0]['bbox']
        x, y, w, h = bbox
        h_img, w_img = image_bgr.shape[:2]
        x = max(0, x)
        y = max(0, y)
        ...
        if crop.size > 0:
            _, buffer = cv2.imencode('.jpg', crop)
            recortes_base64['answer_area_enem'] = base64.b64encode(buffer).decode('utf-8')
    
    resultado['recortes_base64'] = recortes_base64
    return resultado
...
def main():
    ...
    if len(sys.argv) < 2:
        print(json.dumps({'sucesso': False, 'erro': 'Uso: python 02_detectar_rois.py <caminho_imagem>'}))
        sys.exit(1)
